...
var ajsf = {
	...
	*/
	EXTENDS_DOM: {
		...
		*/
		...
		{
			if ( tween && this.tween ){
				this.tween ({opacity:1},{opacity:0},"regularEaseOut",0.3,{onMotionEnd:ajsf.delegate(this,function(){this.empty().detach();})});
			} else {
				this.empty().detach() ;
			}
		},
		...
		show: function ( displayStyle , avoidTween ) {
			if ( !this.isVisible () )
			{
				if ( avoidTween === true || !this.tweenOnDisplay || !this.tween ){
					...
					this.tween ({opacity:0},{opacity:1},"regularEaseOut",0.3);
				}
			}
			return this;
		},
		...
		hide: function ( avoidTween ) {
			if ( this.isVisible () )
			{
				if ( avoidTween === true || !this.tweenOnDisplay || !this.tween ){
					...
					var ref = this; 
					this.tween ({opacity:1},{opacity:0},"regularEaseOut",0.3,{onMotionEnd:function(){ref.hide(true);}});
				}
			}
			return this;
		},
		...
		*/
		...
		/*
			...
			Returns:
		*/
		hasAt: function (attrName) { return (this.hasAttribute ? this.hasAttribute(attrName) : undefined); },
		/*
			Function: hasAts
		...
		*/
		...
		/*
			...
			Returns:
		*/
		getAt: function (attrName) { return this.getAttribute(attrName); },
		...
		/*
			...
			Current instance for chained commands on this element
		*/
		setAt: function (attrName,attrValue) { this.setAttribute(attrName,attrValue); return this; },
		...
		/*
			...
			Current instance for chained commands on this element
		*/
		remAt: function (attrName,attrValue) { this.removeAttribute(attrName); return this; },
		...
		*/
		...
		/*
			...
			Returns:
		*/
		toString: function (){return "[ajsfCoreObject]";},
		...
		*/
		...
		{
			var s, k, 
				f = function(m,p1,p2){ return p1+p2.toUpperCase(); } ;
		...
		*/
		...
		{
			if ( this._lc === true || this._lw > 0 ) { return false; }
			...
			*/
			stringify: function ( obj )
			{
				...
				var str = '' ;
 				...
 				for ( k in obj ) str += k.replace( /([A-Z])/g , function(m,p1){ return '-'+p1.toLowerCase(); } ) + ':' + obj[k] + ';' ;
	...
	*/
	...
	String.prototype.ucfirst = function(){
	   return this.replace( /(^)([a-z])/g , function(m,p1,p2){ return p1+p2.toUpperCase(); } );
	};
